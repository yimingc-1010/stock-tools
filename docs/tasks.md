...
| ST-009 | Universe / Security Master provider + dataset | Done | High | Claude | `feat/main-structure` | UniverseProvider + SecurityMasterDataset + ParquetStore security master I/O |
| ST-010 | Batch price ingestion pipeline | Done | High | Claude | `feat/main-structure` | BatchPriceIngestor with incremental updates, ThreadPoolExecutor, success threshold |
| ST-011 | Real adjustment factor provider | Done | High | Claude | `feat/main-structure` | FinMindAdjustmentProvider using TaiwanStockDividend (cash factor needs price_frame) |
